...
static std::string prefix = "D:/vs2019_repos/shape_based_matching-fusion_by_hand/shape_match_fusion/test/";
...
void my_test(string mode = "test", bool use_rot = true) {
    int num_feature = 32;    // 特征点数量
    line2Dup::Detector detector(num_feature, { 4, 8 });
    if (mode != "test") {
        namedWindow("train", 0);
        Mat img = imread(prefix + "case3/train.jpg");   // 训练图片
        assert(!img.empty() && "check your img path");
        // crop the template from the original train image
        Rect roi(85, 30, 70, 40);
        img = img(roi).clone();
        Mat mask = Mat(img.size(), CV_8UC1, { 255 });
        // padding to avoid rotating out
        int padding = 25;
        ...
        shape_based_matching::shapeInfo_producer shapes(padded_img, padded_mask);
        shapes.angle_range = { 0, 360 };    // 模板库尺度范围
        shapes.angle_step = 1;  // 模板库尺度步长
        //shapes.scale_range = { 0.5f, 1.5f };  // 模板库角度范围
        //shapes.scale_step = 0.1f;    // 模板库角度步长
        shapes.scale_range = { 1.0f };
        shapes.produce_infos();
        std::vector<shape_based_matching::shapeInfo_producer::Info> infos_have_templ;
        string class_id = "gongjian";   // 模板类名
        ...
        for (auto& info : shapes.infos) {
            ...
            // todo: 模板图尺度没有变化的场景
            //（第一次需要成功记录模板，尽量保证模板完整，轮廓层次分明）
            //（如果报错数组越界，先尝试改善模板图片质量，不行再尝试调小num_feature的值，但建议至少为8）
            /*std::cout << "\ninfo.angle: " << info.angle << std::endl;
            int templ_id;
            if (is_first) {
            ...
            std::cout << "templ_id: " << templ_id << std::endl;
            if (templ_id != -1) {
                auto templ = detector.getTemplates("gongjian", templ_id);
                for (int i = 0; i < templ[0].features.size(); i++) {
                    auto feat = templ[0].features[i];
            ...
            {
                is_first = true;
                std::cout << "info_scale: " << info.scale << std::endl;
            }
        }
        detector.writeClasses(prefix + "case3/%s_templ.yaml");  // 模板库
        shapes.save_infos(infos_have_templ, prefix + "case3/gongjian_info.yaml");   // 模板信息
        std::cout << "train end" << std::endl << std::endl;
    }
    else if (mode == "test") {
        namedWindow("test", 0);
        std::vector<std::string> ids;
        ids.push_back("gongjian");
        detector.readClasses(ids, prefix + "case3/%s_templ.yaml");
        // angle & scale are saved here, fetched by match id
        auto infos = shape_based_matching::shapeInfo_producer::load_infos(prefix + "case3/gongjian_info.yaml");

        Mat test_img = imread(prefix + "case3/ori.jpg");   // 测试图片
        assert(!test_img.empty() && "check your img path");
        // padding to avoid rotating out
        int padding = 150;
        cv::Mat padded_img = cv::Mat(test_img.rows + 2 * padding,
        ...
        // cvtColor(img, img, CV_BGR2GRAY); // todo:训练模板用灰度图，测试图片也用灰度图，然后观察推理速度和效果
        std::cout << "test img size: " << img.rows * img.cols << std::endl << std::endl;

        // check time span
        auto start_time = GetTickCount();
        //Timer timer;
        auto matches = detector.match(img, 90, ids);    // 匹配阈值
        //timer.out();
        std::cout << "matches.size(): " << matches.size() << std::endl;
        vector<Rect> boxes;
        vector<float> scores;
        ...
        for (auto match : matches) {
            ...
            auto templ = detector.getTemplates("gongjian",
                match.template_id);
        ...
        cv_dnn::NMSBoxes(boxes, scores, 90, 0, idxs);    // 筛选阈值
        auto end_time = GetTickCount();
        cout << "匹配算法耗时：" << (end_time - start_time) << "ms." << endl;
        ...
        for (auto idx : idxs) {
            ...
            for (size_t i = 0; i < top5; i++) {
                auto match = matches[i];*/
            auto templ = detector.getTemplates("gongjian",  // 读取训练好的模板类
                match.template_id);

            // 65 is width of template image, 35 is height of template image
            // 25 is padding when training
            // tl_x/y: template croping topleft corner when training
            float x_scaled = 60 / 2.0f * infos[match.template_id].scale;
            float y_scaled = 35 / 2.0f * infos[match.template_id].scale;

            // scaling won't affect this, because it has been determined by warpAffine
            // cv::warpAffine(src, dst, rot_mat, src.size()); last param
            float train_img_half_width = 60 / 2.0f + 25;
            float train_img_half_height = 35 / 2.0f + 25;
            ...
            cv::putText(img, to_string(int(round(match.similarity))),
                Point(match.x + x_scaled - 10, match.y - 3), FONT_HERSHEY_PLAIN, 2, randColor);

            cv::RotatedRect rotatedRectangle({ x, y }, { 2 * x_scaled, 2 * y_scaled }, -infos[match.template_id].angle);
